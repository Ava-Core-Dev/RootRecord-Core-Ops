...
Disable: set ``enabled`` false in ``RootRecord Core Ops/Ecoflow/state/ecoflow-ac-solar-gate.json``
...
# Primary gate field (evidence in RootRecord Core Ops/Ecoflow/quota/{DELTA}.json).
...
def _ac_state(data: dict[str, Any]) -> tuple[bool | None, str | None]:
    for key in AC_KEYS:
...
def _verify_ac_state(want_on: bool, *, attempts: int = 3, delay_s: float = 2.0) -> dict[str, Any]:
    observations: list[dict[str, Any]] = []
    for attempt in range(attempts):
        live = _get_live_quota()
        data = live.get("data") or {}
        ac_on, ac_key = _ac_state(data)
        observations.append({"attempt": attempt + 1, "ac_on": ac_on, "ac_key": ac_key})
        if ac_on is not None and ac_on == want_on:
            return {"ok": True, "ac_on": ac_on, "ac_key": ac_key, "observations": observations}
        if attempt + 1 < attempts:
            time.sleep(delay_s)
    return {"ok": False, "error": "ac_state_not_verified", "observations": observations}
...
def evaluate(*, execute: bool = False) -> dict[str, Any]:
    ...
    if put.get("ok"):
        verify = _verify_ac_state(want_on)
        report["verify"] = verify
        if not verify.get("ok"):
            report["ok"] = False
            report["action"] = f"put_unverified_{desired}"
            report["skipped"] = "ac_state_not_verified"
            state["last_decision"] = f"put_unverified_{desired}"
            state["last_skip_reason"] = "ac_state_not_verified"
            log.error(
                "ac-solar-gate PUT AC %s accepted but state was not verified: %s",
                desired.upper(),
                verify,
            )
        else:
            ...
            state["last_skip_reason"] = None
            log.warning(
                "ac-solar-gate PUT AC %s verified input=%.0fW (Starlink on DELTA AC may drop if OFF)",
                desired.upper(),
                input_w,
            )
    else:
